...
export async function GET(req:Request,{params}:{params:{productId:string}}){
  try{
      ...
      return NextResponse.json(store);
         ...
         console.log(`[PRODUCT_GET]`,JSON.stringify(error));
    ...
    if(!sizeId){
      return new NextResponse("colorId is required", { status: 401 });
        ...
        return NextResponse.json(products);
           ...
           console.log(`[PRODUCT_PATCH]`,JSON.stringify(error));
           return new NextResponse('Internal error',{status:500})
       }
}
...
export async function DELETE(req:Request,{params}:{params:{productId:string,storeId:string}}){
    try{
        ...
        const store = await prismadb.product.deleteMany({
           where:{
               id:params.productId,
           }
        });
        return NextResponse.json(store);
           ...
           console.log(`[PRODUCT_DELETE]`,JSON.stringify(error));
           return new NextResponse('Internal error',{status:500})
       }
}